...
import (
	"crypto/rand"
	"github.com/emorydu/component-base/pkg/util/iputil"
	"github.com/emorydu/component-base/pkg/util/stringutil"
...
func randString(letters string, n int) string {
	...
	for p := range output {
		...
		// random % 64
		randomP := random % uint8(l)

		// put into output
		output[p] = letters[randomP]
	}
...
// NewSecretKey returns a secretKey for password.
