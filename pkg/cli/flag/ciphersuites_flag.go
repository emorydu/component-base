...
import (
	"crypto/tls"
	"fmt"
	"github.com/emorydu/component-base/pkg/util/sets"
)
...
func TLSCipherPossibleValues(cipherNames []string) []string {
	cipherKeys := sets.NewString()
...
func TLSCipherSuites(cipherNames []string) ([]uint16, error) {
	...
	for _, cipher := range cipherNames {
		...
		cipherIntSlice = append(cipherIntSlice, intValue)
	}

	return cipherIntSlice, nil
}
...
func TLSPossibleVersions() []string {
	versionKeys := sets.NewString()
	for key := range versions {
		versionKeys.Insert(key)
	}

	return versionKeys.List()
}
